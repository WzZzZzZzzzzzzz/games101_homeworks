...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    float x_arry[3] = {v[0].x(), v[1].x(), v[2].x()};
    float y_arry[3] = {v[0].y(), v[1].y(), v[2].y()};
    std::sort(x_arry, x_arry + 2);
    std::sort(y_arry, y_arry + 2);
    
    int x_min = std::floor(x_arry[0]), x_max = std::ceil(x_arry[2]);
    int y_min = std::floor(y_arry[0]), y_max = std::ceil(y_arry[2]);

    for (int x = x_min; x <= x_max; x++) {
        for (int y = y_min; y <= y_max; y++) {
            if (insideTriangle(x + 0.5f, y + 0.5f, t.v)) {

                auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);float w_reciprocal = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
